...
//    float circle = step(length(uv)+vec2(0.2*sin(u_time), 0.2*cos(u_time))), 0.5);
    float d = length(uv - vec2(0.2*sin(u_time), 0.2*cos(u_time))) - 0.5;
    float md = mod(d, 0.1);
    float nd = abs(d/0.1);
    vec3 col = vec3(0.);
     if(abs(md) < 0.01)
...
void main(void)
{
    ...
    float n = noise(vec2(sin(u_time * 0.3), cos(u_time * 0.2)));
...
void main(void)
{
    ...
    float rect = rectangle(uv, vec2(0.5), vec2(0.05));
//    float circle = step(length(uv)+vec2(0.2*sin(u_time), 0.2*cos(u_time))), 0.5);
    float d = length(uv - vec2(sin(u_time * 0.2), cos(u_time * 0.2))) - 0.5;
    float md = mod(d, mn);
    float nd = abs(d / mn);
    vec3 col = vec3(0.);
     if(abs(md) < 0.2)
...
void main(void)
{
    ...
    float rect = rectangle(uv, vec2(0.5), vec2(0.05));
//    float circle = step(length(uv)+vec2(0.2*sin(u_time), 0.2*cos(u_time))), 0.5);
    float d1 = length(uv - vec2(0.2*sin(u_time * 0.2), 0.2*cos(u_time * 0.2))) - 0.5;
    float d2 = length(uv - vec2(0.5*cos(u_time * 0.5), 0.5*sin(u_time * 0.5))) - 0.25;
//    float d1 = length(uv - vec2(0.2)) - 0.5;
//    float d2 = length(uv + vec2(0.2)) - 0.5;
    float d = min(d1, d2);
    float md = mod(d, mn);
    float nd = abs(d / mn);
    vec3 col = vec3(0.);
...
//    float circle = step(length(uv)+vec2(0.2*sin(time), 0.2*cos(time))), 0.5);
    float d1 = length(uv - vec2(0.2*sin(time * 0.2), 0.2*cos(time * 0.2))) - 0.5;
    float d2 = length(uv - vec2(0.5*cos(time * 0.5), 0.5*sin(time * 0.5))) - 0.25;
//    float d1 = length(uv - vec2(0.2)) - 0.5;
//    float d2 = length(uv + vec2(0.2)) - 0.5;
    float d = max(d1, -d2);
    float md = mod(d, mn);
