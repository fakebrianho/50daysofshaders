...
//    float rnd = noise(uv * time * 0.003);
    ...
    // rotation matrix
    float phi = rnd * pi * 10. + time * 0.4;
    mat2 rot = mat2(cos(phi), -sin(phi), sin(phi), cos(phi));

    vec2 p3 = rot * p2; // apply rotation
//    vec2 p3 = p2;
    p3.y += sin(p3.x * 8. + time * 2.) * 0.12; // wave
    p3.x += cos(p3.y * 2. + time * 2.) * 0.12; // wave
//
    float rep = fract(rnd * 13.285) * 60. + 2.; // line repetition
    float gr = fract(p3.y * p3.x *+ time * 0.0028); // repeating gradient
